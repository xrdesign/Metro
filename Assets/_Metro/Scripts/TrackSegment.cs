...
public class TrackSegment : MonoBehaviour, IMixedRealityPointerHandler
{
  ...
  public Vector3[] cp =
      new Vector3[] { new Vector3(0f, 0f, 0f), new Vector3(0f, 0f, 0f),
  ...
  void Start()
  {
    ...
    lineRenderer.material =
        Resources.Load("Materials/MRTKTransparent") as Material;
  ...
  void Update()
  {
    ...
    if (cp[0] !=
        cp[3]) // Can't bake mesh from line renderer with empty position vector
    {
      try
      {
  ...
  {
    var t1 = 1.0f - t;
    return t1 * t1 * t1 * cp[0] + 3f * t1 * t1 * t * cp[1] +
           3f * t1 * t * t * cp[2] + t * t * t * cp[3];
  }

  public Vector3 Derivative(float t)
  {
    var t1 = 1.0f - t;
    return 3f * t1 * t1 * (cp[1] - cp[0]) + 6f * t1 * t * (cp[2] - cp[1]) +
           3f * t * t * (cp[3] - cp[2]);
  }
