...
{   
    ...
    void Start()
    {
        ...
        liblsl.StreamInfo inf =
            new liblsl.StreamInfo("Eyetracking", "Gaze", 35, 50, liblsl.channel_format_t.cf_float32,
                "ProEye");
        eyeStream = new liblsl.StreamOutlet(inf);
        // event marker for gaze object
        liblsl.StreamInfo markerInfo = new liblsl.StreamInfo("EyetrackingMarker", "Markers", 1, 0,
            liblsl.channel_format_t.cf_string, "ProEyeMarker");
    ...
    void FixedUpdate()
    {
        ...
        Ray rayGlobal = new Ray(Camera.main.transform.position, Camera.main.transform.TransformDirection(GazeDirectionCombinedLocal));
        RaycastHit hit;
        if(gazeRadius > 0)
        {
        ...
        {
            // fill dummy
            hit.point = Camera.main.transform.position + Camera.main.transform.TransformDirection(GazeDirectionCombinedLocal) * 1000;
        }
        ...
        _writer.WriteLine(String.Format("{0:HH:mm:ss.fff}", DateTime.Now) + " - " + Time.time.ToString() +
                          ...
                          ", 3DCombined: " + GazeDirectionCombinedLocal +
                          ", 3DEyeOriginL: " + GazeOriginLeftLocal +
                          ", 3DEyeOriginR: " + GazeOriginRightLocal +
                          ", 3DCombinedOrigin: " + GazeOriginCombinedLocal +
                          ", 3DHit: " + hit.point +
                          ", 3DHead: " + Camera.main.transform.position +
                          ", 3DHeadForward: " + Camera.main.transform.forward +
